"""Does crediting the registration stop markers hurting the seeds that were already good?

Declared before the first run. Mixed world, seeds 0 to 7, 300 m, one thread, MuJoCo, the
calibrated thresholds. Statistic: final along-track error, scheduler against no markers, paired
by seed, with ``OdometryConfig.credit_registration`` off and on. It counts as a fix only if fewer
seeds are made worse by markers with the flag on AND the median paired benefit does not fall by
more than a third. Anything else is reported as not a fix.

    python experiments/credit_registration.py --workers 6
"""
...
def _one(job):
    ...
    r = run_pass(cfg, pol)
    return {"seed": seed, "policy": policy, "credit_registration": int(credit),
...
def main() -> None:
    ...
    with Pool(args.workers, maxtasksperchild=4) as pool:
        rows = sorted(pool.imap_unordered(_one, jobs), key=lambda r: (r["credit_registration"], r["seed"], r["policy"]))
    out = ROOT / "results" / "credit_registration.csv"
    with open(out, "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0]))
        w.writeheader()
        w.writerows(rows)
    for credit in (0, 1):
        by = {(r["policy"], r["seed"]): r["final_along_m"] for r in rows if r["credit_registration"] == credit}
        d = [by[("none", s)] - by[("scheduler", s)] for s in range(args.seeds)]
        med, lo, hi = bootstrap_ci(d)
        print(f"credit_registration={credit}: median none {np.median([by[('none', s)] for s in range(args.seeds)]):.2f} m, "
        ...
        print("   per seed (none, scheduler):", [(s, round(by[("none", s)], 2), round(by[("scheduler", s)], 2)) for s in range(args.seeds)])
    print(f"wrote {out}")
